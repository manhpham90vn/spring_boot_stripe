...
> (thẻ + Konbini + Furikomi), tiền **JPY**. Liên quan: [`payment-stripe-flow.md`](../flows/payment-stripe-flow.md),
> [`saga-purchase-flow.md`](../flows/saga-purchase-flow.md), [`inventory-no-oversell.md`](../flows/inventory-no-oversell.md).
...
|---|--------|-----------------|----|
| 1.1 | Oversell / race | Thao tác **nguyên tử Redis**: GA `DECRBY`, ghế `SET NX`+TTL — chặn oversell ngay tại điểm tranh chấp ([inventory §2](../flows/inventory-no-oversell.md)) | ✅ |
| 1.2 | Reservation TTL | **HOLD có TTL = hạn thanh toán, theo phương thức** (thẻ phút / Konbini-Furikomi ngày) ([inventory §3.1](../flows/inventory-no-oversell.md)) | ✅ |
...
| 2.4 | Tin client thay vì webhook | **Webhook = nguồn sự thật duy nhất** ([saga §2.1](../flows/saga-purchase-flow.md)) | ✅ |
| 2.5 | Session/PI hết hạn | Xử lý `checkout.session.expired` / `payment_intent.canceled` → đóng đơn + nhả hold | 🔧 |
| 2.6 | SCA / 3DS | `requires_action` → chờ webhook, đơn ở AWAITING_PAYMENT ([payment §6](../flows/payment-stripe-flow.md)) | ✅ |
...
| 2.14 | Decline → retry → tiền mồ côi | Thẻ: cho retry trên **cùng PI** trong TTL; khi **bỏ cuộc phải CANCEL PI** rồi mới nhả hold ([saga §3](../flows/saga-purchase-flow.md)) | 🔧 |
...
| 3.2 | Thiếu state machine | **Order state machine** PENDING→AWAITING_PAYMENT→PAID→COMPLETED + REJECTED/CANCELLED ([saga §3](../flows/saga-purchase-flow.md)) | ✅ |
...
| 4.3 | Quản lý secret | Secret từ **env/K8s Secret**, không commit/log; **restricted key**; quy trình rotate ([deployment §6](../ops/deployment-k8s.md)) | ✅ |
...
| # | Vấn đề | Cách giải quyết | TT |
|---|--------|-----------------|----|
| 7.1 | Konbini trả sau, pending lâu | **AWAITING_PAYMENT là bình thường**; job dọn theo **hạn từng phương thức**, không quét nhầm ([saga §2.1](../flows/saga-purchase-flow.md), [inventory §3.1](../flows/inventory-no-oversell.md)) | ✅ |
...
| 7.5 | Trộn sync + async | **Tham số hoá theo phương thức**: TTL giữ kho, hạn `pending`, nhắc khách — mỗi method một bộ ([inventory §3.1](../flows/inventory-no-oversell.md), [payment §7](../flows/payment-stripe-flow.md)) | ✅ |
